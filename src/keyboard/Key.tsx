...
export const Key = ({
  ...
  return (
    <button
      ...
      onClick={onClick}
    >
      <div className={`absolute text-xs ${selected ? "text-primary-content" : "z1text-base-content"} opacity-80 group-hover:opacity-0 top-1 text-nowrap left font-light text-left transition-opacity`}>{header ? shortenHeader(header) : ""}</div>
